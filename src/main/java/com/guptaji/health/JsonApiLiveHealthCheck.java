...
public class JsonApiLiveHealthCheck implements HealthCheck {
  ...
  public HealthCheckResponse call() {
    ...
    // Here we commented the health check of comment API because whenever there will be an error our
    // code on
    // ui-side was not able to show error in which API it is coming so we created another class with
    // @liveness
    // annotation to handle this problem. May be in future i'll get some technique to achieve this
    // in one single
    ...
    // part. but we can also put this under try catch to handle and show the proper error to the
    // user.
